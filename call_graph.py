...
def strip_comments_and_strings(text):
    ...
    Returns cleaned text with same line count (replaced content becomes spaces).
    """
    result = []
    i = 0
    n = len(text)
...
def extract_function_body(lines, start_line_idx):
    ...
    Args:
        ...
        start_line_idx: 0-indexed line where ctags reports the function

    Returns:
        (body_text, line_numbers) where line_numbers[i] is the 1-based source
        line for body_text's i-th character, or (None, None) if parsing fails.
    """
    # Scan forward up to 15 lines to find opening '{'
    brace_start = None
    ...
    # Now track brace depth to find the matching '}'
    depth = 0
    body_chars = []
    body_line_map = []  # maps each char index to 1-based source line
    in_line_comment = False
    in_block_comment = False
    in_string = False
    in_char = False
    ...
    while row < len(lines):
        ...
        while i < len(line):
            ...
            # Handle comments/strings (same logic as strip_comments_and_strings)
            if not in_block_comment and not in_string and not in_char:
            ...
            if ch == '{':
                ...
                if started and depth == 0:
                    ...
                    body_line_map.append(row + 1)  # 1-based
...
def find_calls_in_body(body_text, line_numbers, name_set):
    ...
    Args:
        body_text: function body (with comments/strings stripped)
        line_numbers: maps char index to 1-based source line
        name_set: set of known function names

    Returns:
        list of (callee_name, call_site_line)
    """
    # Strip comments/strings for matching
    cleaned = strip_comments_and_strings(body_text)

    calls = []
    seen = set()  # deduplicate same function called multiple times on same line

    for m in CALL_RE.finditer(cleaned):
        callee = m.group(1)
        if callee not in name_set:
            continue
        # Determine line number from char position
        pos = m.start()
...
def build_call_graph(conn, verbose=False):
    ...
    Returns total number of call relations inserted.
    """
    ...
    for fid, func_list in sorted(functions_by_file.items()):
        ...
        for caller_id, caller_name, func_line, _ in func_list:
            ...
            for callee_name, call_line in calls:
                # Resolve callee IDs (handle duplicate names)
